...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=2,
        help=(
            "Number of rows/cols of local GSPN."
        ),
    )
    parser.add_argument(
...
def main():
    ...
    dismodule = GSPNFusion

    items_each_chunk = args.items_each_chunk
    pure_gspn = False

    all_attn_outputs = []
    all_attn_outputs_teacher = []

    print(f'no checkpoint found in {args.output_dir}, ignoring auto resume')
    config = dismodule.get_default_config(unet=unet)
    if args.mid_dim_scale is not None:
        for each in config["modules_list"]:
            each["projection_mid_dim"] = each["dim_n"] // args.mid_dim_scale
            each["items_each_chunk"] = items_each_chunk
            each["pure_gspn"] = pure_gspn

    model = dismodule(**config)
    model.mount_to(unet=unet)
    ...
    for sub_module in model.modules_list:
    ...
    model, unet, optimizer, train_dataloader = accelerator.prepare(model, unet, optimizer, train_dataloader)
    ...
    for epoch in range(start_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                with torch.no_grad():
                    latents = vae.encode(
                        batch["image"].to(accelerator.device, dtype=weight_dtype) # torch.float32 in the ori
                    ).latent_dist.sample()
            ...
            if global_step % args.save_steps == 0 and accelerator.is_main_process:
                ...
                model.module.save_pretrained(
                    os.path.join(args.output_dir, f"iter-{global_step}"),
                    push_to_hub=False,
                )
