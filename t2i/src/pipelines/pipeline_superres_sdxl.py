...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import StableDiffusionXLHighResPipeline
...
class StableDiffusionXLSuperResPipeline(
    DiffusionPipeline,
    ...
    r"""
    Pipeline for image super-resolution using Stable Diffusion XL.
    ...
    def get_timesteps(self, num_inference_steps, strength, device, denoising_start=None):
        ...
        if denoising_start is None:
            init_timestep = min(math.ceil(num_inference_steps * strength), num_inference_steps)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Args:
            image:
                The image required for super resolution.
            prompt (`str` or `List[str]`, *optional*):
            ...
            cosine_scale_3 (`float`, defaults to 1):
                ...
                in the DemoFusion paper.
            gaussian_sigma (`float`, defaults to 1):
                The standard value of the gaussian filter.
            upscale_strength (`float`, defaults to 0.32):
                The strength of upscale branch, a higher value tends to modify the low-resolution result more
            device (`str` or `torch.device`, *optional*):
                The device to run the pipeline.
        ...
        Returns:
            ...
            `tuple`. When returning a tuple, the first element is a list with the generated images at different scales.
        """
        ...
        if actual_height != height or actual_width != width:
            logger.warning(f"The actual (width, height) used in the model is "
                           f"({actual_width}, {actual_height}), "
                           f"which is different from ({width}, {height}) provided by user")

        original_size = original_size or (actual_height, actual_width)
        target_size = target_size or (actual_height, actual_width)
        ...
        image = self.image_processor.preprocess([image]).to(prompt_embeds)
        ...
        anchor_mean = latents.mean(dim=(2, 3), keepdim=True)
        anchor_std = latents.std(dim=(2, 3), keepdim=True)

        noise_latents = []
        noise = torch.randn_like(latents)
        for timestep in timesteps:
            noise_latent = self.scheduler.add_noise(latents, noise, timestep.unsqueeze(0))
            noise_latents.append(noise_latent.cpu())
        latents = noise_latents[0].to(device)

        self._num_timesteps = len(timesteps)
        backup_param = dict()
        for name, module in self.unet.named_modules():
            if module.__class__.__name__ == 'Conv2d':
                backup_param[name] = (module.dilation, module.padding)
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = latents * (1 - c1) + noise_latents.pop(0).to(device) * c1
                ...
                latents_1 = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]

                for _, module in self.unet.named_modules():
                    if module.__class__.__name__ == 'Conv2d':
                        module.dilation = (num_scales * module.dilation[0], num_scales * module.dilation[1])
                        module.padding = (num_scales * module.padding[0], num_scales * module.padding[1])
                ...
                # expand the latents if we are doing classifier free guidance
                latent_model_input = torch.cat([latents_gaussian] * 2) if self.do_classifier_free_guidance else latents_gaussian
                ...
                latents_dtype = latents.dtype
                self.scheduler._init_step_index(t)
                latents_2 = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]

                c2 = cosine_factor ** cosine_scale_2
                latents = (1 - c2) * latents_1 + c2 * latents_2

                for name, module in self.unet.named_modules():
                    if module.__class__.__name__ == 'Conv2d':
                        module.dilation = backup_param[name][0]
                        module.padding = backup_param[name][1]
        ...
        latents = (latents - latents.mean(dim=(2, 3), keepdim=True)) / latents.std(dim=(2, 3), keepdim=True) * anchor_std + anchor_mean
