...
from ..fusion import LinFusion, GSPNFusion
...
@click.option('--model_name', 'model_name',   help='model name',                    type=str, default='gspn', show_default=True)
...
def main(outdir, subdirs, seeds, max_batch_size, num_fid_samples, text_prompts, repo_id, model_name, device=torch.device('cuda'), use_fp16=True, use_bf16=False, enable_compress_npz=False, num_steps_eval=25, guidance_scale=7.5, resolution=None, custom_seed=False):
    ...
    if model_name == "gspn":
        print("Using GSPN")
        _ = GSPNFusion.construct_for(pipeline)
    elif model_name == "linfusion":
        print("Using LinFusion")
        _ = LinFusion.construct_for(pipeline)
    else:
        print("Using Original Diffusion")
